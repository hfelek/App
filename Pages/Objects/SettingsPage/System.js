...
let SystemParams = Paramsfiltered.find(SystemParams => SystemParams.Tag === "System");
let MenuParams = SystemParams.menu;
const StackSystem = createStackNavigator();

var filtered = Values.filter(row => row.Tag == 'System');
var filteredAT = filtered.filter(row => row.Tag == 'Device Reset');
...
const SystemScreen = ({ route, navigation }) => {
  ...
  function Item(title, value) {
    switch (title) {
      case 'Device Reset':
        return (
          <TouchableOpacity style={styles.itemButton} onPress={() => navigation.navigate('Device Reset')}>
  ...
  const DeviceResetScreen = () => {
    console.log(Values);
    console.log("Values")
    filtered = Values.filter(row => row.Tag == 'System');
    console.log(filtered);
    console.log("filtered")
    console.log(filtered[0].menu)
    filteredAT = filtered[0].menu.filter(row => row.Tag == 'Device Reset');
  ...
  return (
    <StackSystem.Navigator screenOptions={{ headerShown: true, headerTitleAlign: 'center' }}>
      <StackSystem.Screen name='System Main' component={SystemMainScreen} options={{headerTitle:"System"}} />
