...
const ConfigurationBar = ({ config, activeConfig }) => ( <
        ...
        Text style = { styles.title } > { config } < /Text> {
            config == activeConfig && < Text style = {
                { fontSize: 12, color: 'black' } } > { "Active" } < /Text>}
        ...
        function Item(title, value, navigation = null, context = null, parent = null) {
            ...
            switch (title) {
                case 'Configuration 1':
                    ...
                    return ( <
                        TouchableOpacity style = { title == activeConfigEnum ? styles.itemActiveConfig : styles.itemButton }
                        onPress = {
                            () => navigation.navigate('Configuration', { Tag: title, name: title, ConfigNum: parent }) } >
                        <
                        ConfigurationBar activeConfig = { activeConfigEnum }
                ...
                case 'Configuration 3':
                    ...
                    return ( <
                        TouchableOpacity style = { title == activeConfigEnum ? styles.itemActiveConfig : styles.itemButton }
                        onPress = {
                            () => navigation.navigate('Configuration', { Tag: title, ConfigNum: parent, name: title }) } >
                        <
                        ConfigurationBar activeConfig = { activeConfigEnum }
                ...
                case 'Temperature Coefficient Non-Linear':
                    return ( <
                        ...
                        onPress = {
                            () => navigation.navigate('Temperature Coefficient Non-Linear', { Tag: title, ConfigNum: parent }) } >
                        <
                        ItemBar item = { title }
                        /> <
                        /TouchableOpacity>
                ...
                case 'Temperature Coefficient Custom':
                    ...
                    return ( <
                        TouchableOpacity style = { styles.itemButton }
                        onPress = {
                            () => navigation.navigate('Temperature Coefficient Custom', { Tag: title, ConfigNum: parent }) } >
                        <
                        ItemBar item = { title }
                        />
                ...
                case 'Temperature Coefficient Linear':
                    ...
                    return ( <
                        TouchableOpacity style = { styles.itemButton }
                        onPress = {
                            () => navigation.navigate('Temperature Coefficient Linear', { Tag: title, ConfigNum: parent }) } >
                        <
                        ItemBar item = { title }
                        />
        ...
        const TempCoeffMainScreen = ({ navigation }) => {
                ...
                function ConfigurationNumScreen({ route, navigation }) {
                    ...
                    return ( <
                        ...
                        renderItem = {
                            ({ item, index, separators }) => (renderItem(item, navigation, context, Tag)) }
                        keyExtractor = { item => item.Tag }
                        initialNumToRender = { MenuParams.length }
                        /> <
                        /SafeAreaView>
                ...
                const TemperatureCoeffScreen = ({ route, navigation }) => {
                    ...
                    return ( <
                        StackTempCoeff.Navigator screenOptions = {
                            { headerShown: true, headerStyle: styles.headerStyle, headerTitleAlign: 'center' } } >
                        <
                        StackTempCoeff.Screen name = 'MainScreenTempCoeff'
                        component = { TempCoeffMainScreen }
                        options = {
                            { headerTitle: "Temperature Coefficients" } }
                        /> <
                        StackTempCoeff.Screen name = 'Configuration'
                        component = { ConfigurationNumScreen }
                        options = {
                            ({ route }) => ({ headerTitle: route.params.name }) }
                        /> <
                        StackTempCoeff.Screen name = 'Temperature Coefficient Non-Linear'
                        component = { TemperatureCoeffNonLinearScreen }
                        options = {
                            ({ route }) => ({ headerShown: false }) }
                        /> <
                        StackTempCoeff.Screen name = 'Temperature Coefficient Linear'
                        component = { TemperatureCoeffLinearScreen }
                        options = {
                            ({ route }) => ({ headerShown: false }) }
                        /> <
                        StackTempCoeff.Screen name = 'Temperature Coefficient Custom'
                        component = { TemperatureCoeffCustomScreen }
                        options = {
                            ({ route }) => ({ headerShown: false }) }
                        /> <
