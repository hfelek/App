...
const ConfigurationScreen = ({ route, navigation }) => {
    ...
    const BacklightScreen = ({ route, navigation }) => {
            ...
            if (text != Value) {
                navigation.setOptions({
                    headerRight: () => ( <
                        ...
                        onPress = {
                            () => { HandleWriteCommand(peripheralID, "a65373b2-6942-11ec-90d6-024200120000", "a65373b2-6942-11ec-90d6-024200120100", BufferArray(`{'Tag':'Display', 'Set Parameters': {'C3':'${indexValue}'}}`)) } } >
                        <
        ...
        return ( <
            ...
            TouchableOpacity style = { styles.itemButton }
            onPress = {
                () => setText("On") } > { CheckButtoned(text, "On") } <
            /TouchableOpacity> { /* <TouchableOpacity style={styles.itemButton} onPress={() => setText("Off")} > */ } <
            TouchableOpacity style = { styles.itemButton }
            onPress = {
                () => context.setValueByKey("10", "ELiar AHJAS") } >

            { CheckButtoned(text, "Off") } <
            /TouchableOpacity> { ChangedButton(Value, text, navigation) } <
